---
layout: page
title: Bookmarks
---

---

### ASCII

[ascii character codes](https://ss64.com/ascii.html)  

---

### Bash

[bash command line](https://ss64.com/bash/)  
[bash reference](https://www.gnu.org/software/bash/manual/html_node/index.html#SEC_Contents)  
[bash vs powershell](https://web1.cs.wright.edu/~pmateti/Courses/233/Labs/Scripting/bashVsPowerShellTable.html)  

---

### CMD

[cmd command line](https://ss64.com/nt/)  

---

### Colors

[colors tutorial](https://www.w3schools.com/colors/default.asp)  
[color theory](https://www.canva.com/colors/color-wheel/)  
[RYB color picker](https://bahamas10.github.io/ryb/)  
[RGB/HSB color picker](https://www.webfx.com/web-design/color-picker/260036)  
[color picker](https://htmlcolorcodes.com/color-picker/)  
[linux colors](https://misc.flogisoft.com/bash/tip_colors_and_formatting)  
[linux color RGB/HSL definitions](https://jonasjacek.github.io/colors/)  

---

### Docker

[docker](https://docs.docker.com/engine/reference/commandline/docker/)  
[compose](https://docs.docker.com/compose/reference/overview/)  
[kompose](http://kompose.io/)  
[docker hub](https://hub.docker.com/search?q=&type=image)  

---

### Github

[github](https://github.com/)  
[github emoji](https://gist.github.com/rxaviers/7360908)  
[git-extensions documentation](https://git-extensions-documentation.readthedocs.io/en/latest/index.html)  

---

### Go

[gomplate](https://gomplate.hairyhenderson.ca/)  

---

### Helm

[helm](https://helm.sh/docs/helm/)  
[helm hub](https://hub.helm.sh/)  

---

### Hyper-V

[hyperv](https://docs.microsoft.com/en-us/powershell/module/hyper-v/?view=win10-ps)  
[hyperv networking](https://www.altaro.com/hyper-v/complete-guide-hyper-v-networking/)  
[hyperv manager settings explained](https://www.altaro.com/hyper-v/hyper-v-manager-2/virtual-machine-settings/)  

---

### JSON

[json pointer](https://tools.ietf.org/html/rfc6901)  
[json reference](https://tools.ietf.org/html/draft-pbryan-zyp-json-ref-03)  
[json path](https://goessner.net/articles/JsonPath/)  
[jq](https://stedolan.github.io/jq/)  

---

### KubeApps

[kubeapps hub](https://hub.kubeapps.com/)  

---

### Kubernetes

[kompose](http://kompose.io/)  
[kubectl](https://kubernetes.io/docs/reference/kubectl/kubectl/)  
[kubectl cheatsheet](http://wiki.velannes.com/doku.php?id=systeme:kubernetes)  
[kustomize](https://kustomize.io/)  
[kubernetes sonobuoy](https://sonobuoy.io/)  

---

### Linux

[linux man pages](https://www.systutorials.com/docs/linux/man/)  
[rhel 7](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/7/html/)  
[centos 7](https://docs.centos.org/en-US/centos/install-guide/)  
[linux colors](https://misc.flogisoft.com/bash/tip_colors_and_formatting)  
[linux color RGB/HSL definitions](https://jonasjacek.github.io/colors/)  

---

### NethServer

[nethserver](https://www.nethserver.org/documentation/)  

---

### NginX

[nginx](https://nginx.org/en/docs/dirindex.html)  
[nginx-ingress](https://kubernetes.github.io/ingress-nginx/user-guide/nginx-configuration/annotations/)  
[nginx location tester](https://nginx.viraptor.info/)  
[nginx regex tester](https://www.regextester.com/)  

---

### OpenSSL

[openssl certificates](https://jamielinux.com/docs/openssl-certificate-authority/appendix/intermediate-configuration-file.html)  

---

### Packer

[packer](https://www.packer.io/docs/index.html)  

---

### Patch

[patch - strategic-merge-patch](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-api-machinery/strategic-merge-patch.md)  
[patch - json-patch](https://tools.ietf.org/html/rfc6902)  
[patch - json-merge-patch](https://tools.ietf.org/html/rfc7386)  

---

### PowerShell

[powershell command line](https://ss64.com/ps/)  
[powershell reference](https://docs.microsoft.com/en-gb/powershell/module/microsoft.powershell.core/about/about_automatic_variables?view=powershell-6)  
[powershell w10 &amp; wS2016 commands](https://docs.microsoft.com/en-us/powershell/windows/get-started?view=win10-ps)  
[powershell hyper-v commands](https://docs.microsoft.com/en-us/powershell/module/hyper-v/new-vm?view=win10-ps)  
[powershell vmware commands](https://code.vmware.com/docs/7788/cmdlet-reference/doc/index-all_cmdlets.html)  

---

### Swagger

[swaggerhub](https://app.swaggerhub.com/home)  

---

### Terraform

[terraform](https://www.terraform.io/docs/cli-index.html)  
[terraform-plugin-sdk](https://github.com/hashicorp/terraform-plugin-sdk)  

---

### UUID

[uuid generator](https://www.uuidgenerator.net/)  

---

### VMware ESXi

[esxi](https://docs.vmware.com/en/VMware-vSphere/index.html)  
[esxi - esxcli](https://www.virten.net/2018/04/vmware-esxi-6-7-esxcli-command-reference/)  
[esxi - vim-cmd](http://www.doublecloud.org/2013/11/vmware-esxi-vim-cmd-command-a-quick-tutorial)  

---

### YAML

[yq](https://kislyuk.github.io/yq/)  
[yamllint](https://yamllint.readthedocs.io/en/stable/)  

---
